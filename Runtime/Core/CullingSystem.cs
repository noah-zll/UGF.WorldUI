...
namespace UGF.WorldUI
{
    ...
    public class CullingSystem
    {
        ...
        private bool ShouldCullObject(WorldSpaceUIComponent component)
        {
            ...
            var position = GetCullingWorldPosition(component);
            var cameraPosition = Camera.transform.position;
            var isWorldSpaceCanvas = IsWorldSpaceCanvas(component);
            ...
            if (_config.enableFrustumCulling)
            {
                ...
                {
                    if (!WorldSpaceUIUtilities.IsPointInCameraFrustum(position, Camera))
                    {
                        return true;
                    }
                }
            ...
            // 角度剔除
            if (_config.enableAngleCulling && isWorldSpaceCanvas)
            {
        ...
        public void DrawDebugGizmos()
        {
            ...
            foreach (var component in _culledObjects)
            {
                if (component != null)
                {
                    Gizmos.DrawWireCube(GetCullingWorldPosition(component), Vector3.one * 0.5f);
                }
            }
            ...
            foreach (var component in _cullableObjects)
            {
                ...
                {
                    Gizmos.DrawWireCube(GetCullingWorldPosition(component), Vector3.one * 0.3f);
                }
            }
        }
        ...
        private static Vector3 GetCullingWorldPosition(WorldSpaceUIComponent component)
        {
            ...
            {
                offset = component.Config.offset;
            }

            if (component.Config != null && component.Config.followTarget && component.FollowTarget != null)
            {
                return component.FollowTarget.position + offset;
            }

            return component.WorldPosition + offset;
        }

        private static bool IsWorldSpaceCanvas(WorldSpaceUIComponent component)
        {
            var canvas = component?.Group?.GetCanvas();
